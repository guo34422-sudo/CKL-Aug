...
def main():
    ...
    n = 0
    for chunk in reader:
        chunk = chunk[chunk["start"].str.startswith("/c/en/", na=False) &
                      chunk["end"].str.startswith("/c/en/", na=False)]

        chunk["rel"] = chunk["rel"].str.replace(r"^/r/", "", regex=True)
        chunk["start"] = chunk["start"].str.replace(r"^/c/en/", "", regex=True)
        chunk["end"] = chunk["end"].str.replace(r"^/c/en/", "", regex=True)
